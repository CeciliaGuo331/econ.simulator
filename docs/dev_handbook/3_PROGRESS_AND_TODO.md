...
### 世界设计
- [已完成 PR1] 每日 Tick 批处理：管理员界面改为“执行 1 day”，支持输入当日 Tick 数。
- [已完成 PR1] 日终脚本轮换：日终边界替换脚本代码，实体状态保留，下一交易日生效。
...
- [已完成] 仪表盘功能拆分：用户/管理员 Tab 化与默认兼容；链接修复与配色可读性提升。
...
1. **每日 Tick 调度 & 日终脚本换代（PR2 扩展）**
	- 批量轮换与回滚：支持选择多脚本、查看历史版本并一键回滚。
	- 管理端提示日终状态与快捷“运行下一天”操作。
	- 服务端记录审计日志，暴露查询接口。
2. **交易与主体状态模型重构**
	- 扩展 Redis `sim:{id}:state` 与 runtime 存储以容纳交易撮合、账户流水（基础设施已就绪：runtime/trades/ledger List/JSON）。
	- PostgreSQL 已引入 `script_versions` 与 `agent_snapshots`（草案）；后续补齐交易/账本归档表与查询 API。
	- 更新数据同步策略：界定 `assign` / `delta` 的使用场景，推进写穿或双写方案落地。
3. **运行期脚本安全护栏**
