...
class PostgresScriptStore:
    ...
    async def save_script(self, metadata: ScriptMetadata, code: str) -> None:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                f"""
                INSERT INTO {qualified} (script_id, simulation_id, user_id, description, created_at, code, code_version)
    ...
    async def list_all_metadata(self) -> List[ScriptMetadata]:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                f"""
                SELECT script_id, simulation_id, user_id, description, created_at, code_version
                FROM {qualified}
                ORDER BY created_at
                """
            )
